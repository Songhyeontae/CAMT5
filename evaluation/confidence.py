...
def get_perplexity(
    ...
    best_sequence = outputs.sequences.reshape(batch_size, num_sequences,
                                              -1)[:, 0, :]
    ...
    label_ids[
        label_ids == tokenizer.
        pad_token_id] = IGNORE_INDEX  # Ignore padding token (cross-entropy loss)
    ...
    with torch.no_grad():
        ...
        loss_per_token = cross_entropy(logits.view(-1, logits.size(-1)),
                                       label_ids.view(-1),
                                       ignore_index=IGNORE_INDEX,
                                       reduction="none").view(batch_size, -1)
        sequence_lengths = label_attention_mask.sum(dim=1)
        length_penalty = (sequence_lengths /
                          (sequence_lengths**length_normalize))
...
def get_entropy(
    ...
    sentence_log_probs = transition_scores.sum(dim=1).reshape(
...
def get_len_norm_entropy(
    ...
    sequence_lengths = (outputs.sequences != pad_token_id).sum(dim=1).reshape(
        batch_size, -1)  # (batch_size, num_sequences)
    sentence_log_probs = transition_scores.sum(dim=1).reshape(
        batch_size, -1)  # (batch_size, num_sequences)
    importance_sampling_weight = torch.nn.functional.softmax(
...
def get_importance_weighted_entropy(
    ...
    sentence_log_probs = weighted_transition_scores.sum(dim=1).reshape(
        batch_size, -1)  # (batch_size, num_sequences)
    importance_sampling_weight = torch.nn.functional.softmax(
        sentence_log_probs / 0.01, dim=1)
    entropy = -1 * (sentence_log_probs *
...
def get_rdk(
    ...
    decoded_prediction = [representation.decode(pred) for pred in prediction]
