...
class DataCollatorForUnimptT5:
    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> BatchEncoding:
        ...
        if isinstance(examples[0], tuple):
            examples = [
                sample_dict for sample_tuple in examples
                for sample_dict in sample_tuple
            ]
        ...
        batch_src_tgt = BatchEncoding({
            k: np.array(
                [examples_src_tgt[i][k] for i in range(len(examples_src_tgt))])
            for k, v in examples_src_tgt[0].items()
        })
        batch_src_tgt['labels'][batch_src_tgt['labels'] ==
                                self.pad_token_id] = -100
        # pad batch['labels'] to the same the batch['input_ids']
        batch['labels'] = np.concatenate(
            (batch['labels'],
             np.full(
                 (batch_size, self.input_length - self.target_length), -100)),
            axis=1)

        batch = {
            k: np.concatenate((batch[k], batch_src_tgt[k]), axis=0)
            for k in batch
        }
...
class MixedDataset(IterableDataset):
    ...
    def __init__(
        ...
        text2mol_dataset: Dataset,
    ):
        self.text_dataset = text_dataset
        self.mol_dataset = mol_dataset
        self.text2mol_dataset = text2mol_dataset
    ...
    def __iter__(self) -> Iterator[Tuple[Dict[str, Any], Dict[str, Any]]]:
        ...
        text2mol_iter = iter(self.text2mol_dataset)
        ...
        while True:
            ...
            molecule_batch = self._reset_iterator(molecule_iter,
                                                  self.mol_dataset)
            text2mol_batch = self._reset_iterator(text2mol_iter,
                                                  self.text2mol_dataset)

            # Return the combined batch
            yield text_batch, molecule_batch, text2mol_batch
