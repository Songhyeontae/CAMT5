...
NO_DECAY_PARAM_NAME_PATTERN = re.compile(
    "|".join(["bias", "LayerNorm", "layernorm", "layer_norm", "ln"])
)

def _weight_decay_excluded_params(model: PreTrainedModel, weight_decay: float) -> List[nn.parameter.Parameter]:
    ...
    for name, param in named_parameters:
        parameters.append({
            "params": param,
            "weight_decay": 0.0 if NO_DECAY_PARAM_NAME_PATTERN.search(name) else weight_decay,
        })
    
    return parameters

def get_optimizer(model: PreTrainedModel, optim_confg: OptimConfig) -> torch.optim.Optimizer:
